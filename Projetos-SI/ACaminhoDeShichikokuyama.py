    ...
    for silaba in silabas:  # cada silaba
        if silaba in hospital:  # se silaba em hospital
            ...
            if indice_nome not in silaba_achada:
                ...
                # add no tem_hospital a silaba mandada
                tem_hospital.append(silaba)
# prints iniciais
    if len(tem_hospital) == 1:
        print(
            f'Lembrei! A sílaba {tem_hospital[0]} está no nome do hospital. Obrigada, Totoro!')
    elif len(tem_hospital) > 1:
        em_ordem = na_ordem(tem_hospital)
        t = ''
        if em_ordem and t.join(tem_hospital) == silaba_input:
            print(
                f'A palavra {t.join(tem_hospital)} está toda no nome do hospital. Acertou em cheio, Totoro!')
        else:
            separador = ', '
            print(
                f'Lembrei! As sílabas: {separador.join(tem_hospital)} estão no nome do hospital. Obrigada, Totoro!')
